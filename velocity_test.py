...
class BaseConfig:
    ...
    batch_size: int = 245760 #int(np.prod(image_shape)) #int(np.prod(image_shape)) if len(image_shape) < 4 else 1 #743424 # 28 * 28  #21023600 for 3D mri #80860 for 2D mri#784 for MNIST #2500 for GPU mem ?
    epochs: int = 500
...
indices = torch.linspace(start=0, end=1, steps=len(verts)) #maybe you will have to 0, 1 it
...
for i in range(t_range):
    Y = torch.cat((Y, torch.FloatTensor(cube_list[i].vertices / 186))) #TODO better normalisation

#F(idx, t) = (x, y, z)
dataset = TensorDataset(X, Y)

train_loader = DataLoader(dataset, batch_size=config.batch_size, shuffle=True, num_workers=os.cpu_count())

#Siren
model = SirenNet(dim_in=config.dim_in, dim_hidden=config.dim_hidden, dim_out=config.dim_out, num_layers=config.num_layers, lr=config.lr)
...
for i in range(10):
    ...
    #create mesh
    predmesh = trimesh.Trimesh(vertices=pred.detach().cpu().numpy(), faces=faces, vertex_normals=normals)
    predmesh.show()
