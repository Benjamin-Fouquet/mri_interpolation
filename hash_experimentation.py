...
class BaseConfig:
    checkpoint_path = (
        None  #'lightning_logs/version_61/checkpoints/epoch=199-step=12000.ckpt'
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dim_hidden", help="hidden dimension for decoder", type=int, required=False
    )
    parser.add_argument("--n_levels", help="number of levels", type=int, required=False)
    parser.add_argument(
        "--n_features_per_level", help="n_features_per_level", type=int, required=False
    )
    parser.add_argument(
        "--num_layers", help="number of layers for decoder", type=int, required=False
    )
    parser.add_argument(
        "--dropout", help="Dropout for decoder", type=float, required=False
    )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.encoder_type == "tcnn":
            self.encoder = torch.nn.utils.parametrizations.spectral_norm(
                tcnn.Encoding(
...
tinyY = Y = torch.FloatTensor(data[..., [3, 7, 13]]).reshape(
    -1, 1
)  # TODO: Activate this for brain
