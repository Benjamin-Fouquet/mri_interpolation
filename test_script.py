...
data_acc_grad = nib.load('/home/aorus-users/Benjamin/git_repos/mri_interpolation/results_siren/255/training_result.nii.gz').get_fdata()
data_noacc_grad = nib.load('/home/aorus-users/Benjamin/git_repos/mri_interpolation/results_siren/254/training_result.nii.gz').get_fdata()
gt = nib.load('/home/aorus-users/Benjamin/git_repos/mri_interpolation/results_siren/248/ground_truth.nii.gz').get_fdata()
gt = (gt - np.min(gt)) / (np.max(gt) - np.min(gt)) * 2 - 1

diff_gt_acc = data_acc_grad - gt
diff_gt_noacc = data_noacc_grad - gt

nib.save(nib.Nifti1Image(diff_gt_acc, affine=np.eye(4)), 'diff_acc.nii.gz')
nib.save(nib.Nifti1Image(diff_gt_noacc, affine=np.eye(4)), 'diff_noacc.nii.gz')
