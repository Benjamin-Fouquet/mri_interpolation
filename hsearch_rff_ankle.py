'''
hyperparameter search for random fourier features using optuna
'''
...
study_name = "rffMLP_study"  # Unique identifier of the study.
storage_name = f"sqlite:///{study_name}.db"

def objective(trial):
    #parameters to search
    config.n_frequencies = trial.suggest_int("n_frequencies", 150, 3000)
    config.sigma = trial.suggest_float("sigma", 2, 50)
    config.n_frequencies_t = trial.suggest_int("n_frequencies_t", 5, 30)
    config.sigma_t = trial.suggest_float("sigma_t", 1, 20)
...
with open(filepath + 'best_params_rffMLP.txt', 'w') as f:
    print(study.best_params, file=f)

study.trials_dataframe().to_csv(filepath + 'rffMLP_tests.csv')
