...
from models import (AutoEncoder, ConvModule, ThreeDCNN, UnetAdded,
                    UnetConcatenated)
...
class MriDataModule(pl.LightningDataModule):
    ...
    def setup(self):
        all_t2s = glob.glob(self.dhcp_path + "*_t2_seg.nii.gz", recursive=True)[
            : self.max_subjects
        ]
        subjects = []
        down_factor = 3.0
        ...
        all_baboons = glob.glob(self.baboon_path + "*.nii.gz", recursive=True)[
            : self.max_subjects
        ]
        baboons = []
        for path in all_baboons:
            baboon_image = tio.RescaleIntensity(out_min_max=(0, 1))(
        ...
        if self.patch_size is None:
            self.train_ds, self.val_ds = torch.utils.data.random_split(
                dataset=subjects_dataset, lengths=[self.train_length, self.val_length]
            )
            ...
            self.train_ds, self.val_ds = torch.utils.data.random_split(
                ...
                lengths=[
                    self.train_length * samples_per_volume,
                    self.val_length * samples_per_volume,
                ],
            )
    ...
    def train_dataloader(self) -> DataLoader:
        return DataLoader(
            ...
            num_workers=multiprocessing.cpu_count() if not self.patch_size else 0,
        )
...
trainer.fit(
    model,
    train_dataloaders=mri_dataloader.train_dataloader(),
    val_dataloaders=mri_dataloader.val_dataloader(),
)
...
if isinstance(model, UnetConcatenated):
    baboon_image = torch.FloatTensor(
        np.vstack([baboon_image[:, :96, :], np.zeros((2, 96, 27))])
    )
# pred = model(baboon_image[:, :, 13].unsqueeze(0).unsqueeze(0).unsqueeze(-1))
pred = model.forward(
    baboon_image[:, :, 13].unsqueeze(0).unsqueeze(0).squeeze(-1)
)  # TODO: correct the ugly code ACHTUNG: do not call forwar direclty. Call model(x) instead
