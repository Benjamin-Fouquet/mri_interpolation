...
from models import ModulatedSirenNet, Modulator
...
class BaseConfig:
    ...
    w0: float = 2.8
    n_frequencies: int = (
        352  # for classic, n_out = 2 * n_freq. For tcnn, n_out = 2 * n_freq * dim_in
    )
    sigma: float = 10
    w0_t: float = 26.5
    ...
    lr: float = (
        0.005  # G requires training with a custom lr, usually lr * 0.1 #10-4 for Siren
    )
    interp_factor: int = 2
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--encoder_type", help="tcnn or classic", type=str, required=False
    )
    parser.add_argument(
        "--n_frequencies",
        help="number of encoding frequencies",
        type=int,
        required=False,
    )
    parser.add_argument(
        "--n_frequencies_t",
        help="number of encoding frequencies for time",
        type=int,
        required=False,
    )
    parser.add_argument(
        "--dim_hidden", help="size of hidden layers", type=int, required=False
    )
    parser.add_argument(
        "--num_layers", help="number of layers", type=int, required=False
    )
...
class ComplexGaborLayer(torch.nn.Module):
    ...
    def __init__(
        ...
        self.linear = torch.nn.Linear(dim_in, dim_out, bias=bias, dtype=dtype)
...
class GaborNet(pl.LightningModule):
    def __init__(
        self, dim_in, dim_hidden, dim_out, n_layers, sigma, w0, lr, *args, **kwargs
    ):
        super().__init__()
        ...
        for i in range(self.n_layers):
            layers.append(
                self.layer_cls(
                    dim_in=self.dim_in if i == 0 else self.dim_hidden,
                    dim_out=self.dim_out if i == (n_layers - 1) else self.dim_hidden,
                    c=self.sigma,
                    w0=self.w0,
                )
            )
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.final_loss = float(
            loss.detach().cpu().numpy()
        )  # parameter used for optuna
        return loss
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.skip_connections = (
            skip_connections  # index of skip connections, starting from 0
        )
        self.lr = lr
        self.w0 = w0
        ...
        if self.encoder_type == "gabor":
            self.encoder = torch.nn.Sequential(
                RealGaborLayer(
                    ...
                    dim_out=self.n_frequencies,
                    is_first=True,
                    w0=self.w0,
                    c=self.sigma,
                ),
                RealGaborLayer(
                    dim_in=self.n_frequencies,
                    dim_out=self.n_frequencies,
                    w0=self.w0,
                    c=self.sigma,
                ),
            )
            self.encoder_t = RealGaborLayer(
                dim_in=1,
                dim_out=self.n_frequencies_t,
                is_first=True,
                w0=self.w0_t,
                c=self.sigma_t,
            )
        ...
        if self.encoder_type == "siren":
            ...
            self.encoder = ModulatedSirenNet(
                dim_in=(self.dim_in - 1),
                dim_hidden=self.n_frequencies,
                dim_out=self.n_frequencies,
                num_layers=2,
                w0_initial=self.w0,
                w0=self.w0,
            ...
            self.encoder_type == "tcnn"
        ):  # if tcnn is especially required, set it TODO: getattr more elegant
            self.encoder = tcnn.Encoding(
                n_input_dims=(self.dim_in - 1),
                encoding_config={
                    "otype": "Frequency",
            ...
            self.encoder_t = tcnn.Encoding(
                n_input_dims=1,
                encoding_config={
                    "otype": "Frequency",
                    "n_frequencies": self.n_frequencies_t,
                },
                dtype=torch.float32,
            )
            self.encoding_dim_out = (
                self.n_frequencies * 2 * (self.dim_in - 1) + self.n_frequencies_t * 2
            )
            ...
            self.encoder = rff.layers.GaussianEncoding(
                sigma=self.sigma,
                input_size=(self.dim_in - 1),
                encoded_size=self.n_frequencies,
            )
            self.encoder_t = rff.layers.GaussianEncoding(
                sigma=self.sigma_t, input_size=1, encoded_size=self.n_frequencies_t
            )
        ...
        for i in range(self.n_layers):
            ...
            block = torch.nn.Sequential(
                ...
                ),  # you can do batchnorm 3D + 1D and cat after
                # torch.nn.ReLU()
                torch.nn.GELU(),
            )
