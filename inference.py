...
if __name__ == '__main__':
    ...
    if args.test_image is None:
        ...
        if args.input.split('/')[1]=='mnt' and args.mode=='reconstruction':
            sub['T1_image'].save('/home/aorus-users/claire/LR.nii.gz')
            sub['T2_image'].save('/home/aorus-users/claire/HR.nii.gz')
        elif args.input.split('/')[1]=='mnt' and args.mode=='degradation':
            sub['T1_image'].save('/home/aorus-users/claire/HR.nii.gz')
            sub['T2_image'].save('/home/aorus-users/claire/LR.nii.gz')
        else:
            sub['T1_image'].save('/home/aorus-users/claire/LR.nii.gz')
            sub['T2_image'].save('/home/aorus-users/claire/HR.nii.gz')
                    ...
                    if gpu is not None:
                        if network=="Disentangled_plusplus":
                            ...
                            if compteur==160:
                                ...
                                plt.colorbar()
                                plt.savefig('/home/aorus-users/claire/Inference.png')
                                plt.close()
                            compteur=compteur+1
                        else:
    ...
    if args.ground_truth is not None:
        ...
        # print(psnr)
        # print(psnr_base)
        # p=PSNRMetric(max_val=1)
        # print(((torch.squeeze(pred_image.data,0)-min_pred)/(max_pred-min_pred)).shape)
        # print(((torch.squeeze(gt_image.data,0)-min_gt)/(max_gt-min_gt)).shape)
        # print(p((torch.squeeze(pred_image.data,0)-min_pred)/(max_pred-min_pred),(torch.squeeze(gt_image.data,0)-min_gt)/(max_gt-min_gt)))
        # print(p((torch.squeeze(pred_image.data,0)-min_pred)/(max_pred-min_pred),(torch.squeeze(gt_image.data,0)-min_gt)/(max_gt-min_gt)).shape)
