...
class MriDataModule(pl.LightningDataModule):
    ...
    def __init__(self, image_path: str, batch_size: int, *args, **kwargs):
        super().__init__()
        self.train_ds = None
        self.val_ds = None
        self.test_ds = None
...
ground_truth = ground_truth_image.get_fdata(
    dtype=np.float32
)  # [:,:,int(ground_truth.shape[-1] / 2)] [64:128, 64:128, 96]
...
if config.dim_in == 3:
    ...
    mgrid = torch.stack(torch.meshgrid(x, y, z), dim=-1)
    mgrid = torch.FloatTensor(mgrid).reshape(
        ground_truth_image.shape[0]
        * ground_truth_image.shape[1]
        * ground_truth_image.shape[2],
        config.dim_in,
    )
...
if config.dim_in == 2:
    ...
    mgrid = torch.stack(torch.meshgrid(x, y), dim=-1)
    mgrid = torch.FloatTensor(mgrid).reshape(
        ground_truth_image.shape[0] * ground_truth_image.shape[1], config.dim_in
    )
...
# nifti here
nib.save(
    nib.Nifti1Image(pred, ground_truth_image.affine), filepath + "prediction.nii.gz"
)
if config.dim_in == 3:
...
if config.dim_in == 3:
    ...
    mgrid = torch.stack(torch.meshgrid(x, y, z), dim=-1)
    mgrid = torch.FloatTensor(mgrid).reshape(
        config.output_size * config.output_size * config.output_size, config.dim_in
    )
...
if config.dim_in == 2:
    ...
    mgrid = torch.stack(torch.meshgrid(x, y), dim=-1)
    mgrid = torch.FloatTensor(mgrid).reshape(
        config.output_size * config.output_size, config.dim_in
    )

up_dataset = torch.utils.data.TensorDataset(mgrid, mgrid)
up_loader = torch.utils.data.DataLoader(
    up_dataset, batch_size=config.batch_size, num_workers=config.num_workers
)
...
if config.dim_in == 3:
    upsampling = upsampling.reshape(
        config.output_size, config.output_size, config.output_size
    )
upsampling = upsampling.detach().numpy()
# nifti here
nib.save(
    nib.Nifti1Image(upsampling, ground_truth_image.affine),
    filepath + "upsampling.nii.gz",
)
if config.dim_in == 3:
