...
study_name = "siren_net_study"  # Unique identifier of the study.
storage_name = f"sqlite:///{study_name}.db"

def objective(trial):
    n_layers = trial.suggest_int("n_layers", 1, 10)
    dim_hidden = trial.suggest_int("dim_hidden", 64, 1024, 64)
...
with open(filepath + 'best_params.txt', 'w') as f:
    print(study.best_params, file=f)

study.trials_dataframe().to_csv(filepath + 'siren_tests.csv')