...
import copy
import config as cg
from models import SirenNet, PsfSirenNet
import datamodules
import optimizers
...
if __name__ == '__main__':
    ...
    parser.add_argument('--accumulate_grad_batches', help='number of batches accumulated per gradient descent step', type=int, required=False)
    parser.add_argument('--n_sample', help='number of points for psf in x, y, z', type=int, required=False)
    parser.add_argument('--model_class', help='Modele class selection', type=str, required=False)
...
class Config:
    ...
    lr: float = 1e-5 #G requires training with a custom lr, usually lr * 0.1
    datamodule: pl.LightningDataModule = MriDataModule
    model_cls: pl.LightningModule = SirenNet
